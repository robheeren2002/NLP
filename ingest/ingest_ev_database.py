...
FILTER_URLS = [r"^https://ev-database\.org/car/\d{4}/Volvo-.*$"]

POLestar_URL_RE = re.compile(r"^https?://ev-database\.org/(?:uk/)?car/\d+/Polestar", re.IGNORECASE)
...
OUTPUT_FILENAME = "docs_web_map_ev_database_volvo.json"
...
if True:
    ...
    # ✅ Proper Polestar filtering (regex) with minimal code change:
    #docs_sitemap = [d for d in docs_sitemap if POLestar_URL_RE.match(d.metadata.get("source", ""))]

    print(f"📄 Loaded {len(docs_sitemap)} Polestar document(s) from EV-Database sitemap")
    for doc in docs_sitemap:
        doc.metadata["tag_source"] = "ev_database"
        doc.metadata["brand"] = "polestar"
else:
